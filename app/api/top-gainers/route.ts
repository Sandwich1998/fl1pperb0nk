...
export async function GET(req: Request) {
  ...
  try {
    const [latest, summary24h, mapping] = await Promise.all([
      getCachedJson<LatestResponse>("top-gainers-latest", "/latest", 30_000),
      getCachedJson<Summary24hResponse>("top-gainers-24h", "/24h", 60_000),
      getCachedJson<MappingItem[]>("top-gainers-mapping", "/mapping", 60 * 60 * 1000),
    ]);
    ...
    return NextResponse.json({ error: "Failed to load top gainers" }, { status: 500 });
  }
}
